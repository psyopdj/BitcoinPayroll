...
class Shift:
    ...
    def to_json(self):
        json_dct = {"employee_id": self.employee_id, "date": self.date, "in_timestamp": self.in_timestamp,
                    "out_timestamp": self.out_timestamp, "transaction_id": self.transaction_id}
    ...
    def from_json_dict(json_dict):
        return Shift(json_dict.get('_id', None), json_dict['employee_id'], json_dict['date'], json_dict['in_timestamp'],
                     json_dict['out_timestamp'], json_dict['transaction_id'])
    ...
    def __hash__(self):
        return (
            self._id, self.employee_id, self.date, self.in_timestamp, self.out_timestamp,
            self.transaction_id).__hash__()
    ...
    def __eq__(self, other):
        if isinstance(other, Shift):
            return self._id == other._id and \
                   ...
                   self.out_timestamp == other.out_timestamp and \
